  ...
  return (
    <motion.div {...bubbleMotion} className="flex flex-col items-end gap-1">
      <div className="flex justify-end items-end gap-2">
        <div className="bg-thera-stabilite text-white p-4 rounded-2xl rounded-tr-md max-w-[80%] text-sm leading-relaxed whitespace-pre-wrap">
          {content}
        </div>
      </div>
  ...
  return (
    <motion.div {...bubbleMotion} className="flex flex-col items-start gap-1">
      <div className="flex justify-start items-start gap-3">
        <Avatar isError={isError} />
        <div
          className={`${
            isError
              ? "bg-red-50 text-thera-technologie border border-thera-technologie/20"
              : "bg-thera-confiance/60 text-thera-stabilite border border-thera-stabilite/5"
...
export function TypingIndicator() {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex justify-start items-start gap-3">
      <Avatar />
      <div className="bg-thera-confiance/60 p-4 rounded-2xl rounded-tl-md border border-thera-stabilite/5 flex gap-1.5 items-center">
        {[0, 1, 2].map((i) => (
          <motion.span
            key={i}
            className="w-1.5 h-1.5 bg-thera-stabilite/40 rounded-full"
